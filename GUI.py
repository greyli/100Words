...
def save_command():
    ...
    if length > 99:
        ...
        content = ls + today + ls + str(length) + " words" + ls + data
        file.write(content)
        file.close()
...
def about_command():
    ...
    top.title("About 100Words")
    
    label1 = Tkinter.Label(top, text="""100Words\nv1.1
...
def rule_command():
    ...
    rule = Tkinter.Toplevel()
    rule.title("Rules")
    rule.geometry("620x200")
    
    canvas = Tkinter.Canvas(rule)
    canvas.create_text(20, 30, anchor=Tkinter.W, font="Purisa 20 bold",
        text="Three Rules")
    canvas.create_text(20, 80, anchor=Tkinter.W, font="Purisa 16",
        text="Rule 1: You must write no less than 100 words.")
    canvas.create_text(20, 110, anchor=Tkinter.W, font="Purisa 16",
        text="Rule 2: You can only use one emoji in one day.")
    canvas.create_text(20, 140, anchor=Tkinter.W, font="Purisa 16",
        text="Rule 3: You can not change the previous diary.")
...
command=save_command,bg="#d6e685", fg="black")
...
helpmenu.add_command(label="100Words Help", command=detail_command)
